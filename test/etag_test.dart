...
void main() {
  group('etag(entity)', () {
    ...
    group('when "entity" is a Uint8List', () {
      test('should generate a strong ETag', () {
        expect(etag(Uint8List.fromList([1, 2, 3])), '"3-cDeAcZjCKn0rCAc3HXY3eahP388"');
      });
    ...
    group('when "entity" is a FileStat', () {
      test('should generate a weak ETag', () {
        expect(etag(File(Platform.resolvedExecutable).statSync()), startsWith('W/'));
      });

      test('should generate consistently', () {
        expect(etag(File(Platform.resolvedExecutable).statSync()), etag(File(Platform.resolvedExecutable).statSync()));
      });
    });
    ...
    group('with "weak" option', () {
      group('when "false"', () {
        test('should generate a strong ETag for a string', () {
          ...
          expect(etag('beep boop', weak: false), '"9-fINXV39R1PCo05OqGqr7KIY9lCE"');
          // expect(etag(str5kb, weak: false), '"1400-CH0oWYLQGHe/yDhUrMkMg3fIdVU"');
        });

        test('should generate a strong ETag for a Buffer', () {
          expect(etag(Uint8List.fromList([]), weak: false), '"0-2jmj7l5rSw0yVb/vlWAYkK/YBwk"');
          expect(etag(Uint8List.fromList([1, 2, 3]), weak: false), '"3-cDeAcZjCKn0rCAc3HXY3eahP388"');
          // expect(etag(buf5kb, weak: false), '"1400-CH0oWYLQGHe/yDhUrMkMg3fIdVU"');
        });

        test('should generate a strong ETag for fs.Stats', () {
          expect(etag(File(Platform.resolvedExecutable).statSync(), weak: false), isNot(startsWith('W/')));
        });
      });
      ...
      group('when "true"', () {
        test('should generate a weak ETag for a string', () {
          ...
          expect(etag('beep boop', weak: true), 'W/"9-fINXV39R1PCo05OqGqr7KIY9lCE"');
          // expect(etag(str5kb, weak: false), '"W/1400-CH0oWYLQGHe/yDhUrMkMg3fIdVU"');
        });

        test('should generate a weak ETag for a Buffer', () {
          expect(etag(Uint8List.fromList([]), weak: true), 'W/"0-2jmj7l5rSw0yVb/vlWAYkK/YBwk"');
          expect(etag(Uint8List.fromList([1, 2, 3]), weak: true), 'W/"3-cDeAcZjCKn0rCAc3HXY3eahP388"');
          // expect(etag(buf5kb, weak: false), '"1400-CH0oWYLQGHe/yDhUrMkMg3fIdVU"');
        });

        test('should generate a weak ETag for fs.Stats', () {
          expect(etag(File(Platform.resolvedExecutable).statSync(), weak: true), startsWith('W/'));
        });
      });
    });
  });
